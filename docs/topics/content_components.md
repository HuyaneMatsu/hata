...
Additionally, you can create a small space between them (about half the size of a line break) by using multiple of them,
one after the other.
...
Thumbnails can have an alternative description, which can also be marked as a spoiler. 
...
By using `MediaGalleryItem`-s you can assign additional alternative description to each media and also mark them
as spoiler.
