__all__ = ('InteractionComponentMetadataTextInput',)

from scarletio import copy_docs, include

from .base import InteractionComponentMetadataBase
from .fields import parse_custom_id, parse_value, put_custom_id, put_value, validate_custom_id, validate_value

ComponentType = include('ComponentType')


class InteractionComponentMetadataTextInput(InteractionComponentMetadataBase):
    """
    Interaction component metadata representing a text input component.
    ...
    def iter_custom_ids_and_values(self):
        custom_id = self.custom_id
        if (custom_id is not None):
            yield (custom_id, ComponentType.text_input, self.value)
