...
def test__EmbedImage__repr():
    ...
    vampytest.assert_instance(repr(embed_image), str)
...
def test__EmbedImage__hash():
    ...
    embed_image = EmbedImage(url)
    vampytest.assert_instance(hash(embed_image), int)
...
def test__EmbedImage__eq(
    ...
    output : `bool`
    """
    embed_image_0 = EmbedImage(**keyword_parameters_0)
    for item in additional_attributes_0.items():
        setattr(embed_image_0, *item)
    
    
    embed_image_1 = EmbedImage(**keyword_parameters_1)
    for item in additional_attributes_1.items():
        setattr(embed_image_1, *item)
    
    output = embed_image_0 == embed_image_1
...
def test__EmbedImage__bool(keyword_parameters):
    ...
    output : `bool`
    """
    embed_image = EmbedImage(**keyword_parameters)
    output = bool(embed_image)
    vampytest.assert_instance(output, bool)
    return output
