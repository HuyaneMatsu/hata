from datetime import datetime as DateTime, timezone as TimeZone

import vampytest

from ....utils import datetime_to_timestamp

from ..fields import put_started_at


def _iter_options():
    started_at = DateTime(2016, 5, 14, tzinfo = TimeZone.utc)
    
    yield (
        None,
        False,
        {},
    )
    
    yield (
        None,
        True,
        {
            'created_at': None,
        },
    )
    
    yield (
        started_at,
        False,
        {
            'created_at': datetime_to_timestamp(started_at),
        },
    )
    
    yield (
        started_at,
        True,
        {
            'created_at': datetime_to_timestamp(started_at),
        },
    )


@vampytest._(vampytest.call_from(_iter_options()).returning_last())
def test__put_started_at(input_value, defaults):
    """
    Tests whether ``put_started_at`` works as intended.
    
    Parameters
    ----------
    input_value : `None | DateTime`
        The input value to serialize.
    
    defaults : `bool`
        Whether values with their default value should be included in the output data.
    
    Returns
    -------
    output : `dict<str, object>`
    """
    return put_started_at(input_value, {}, defaults)
