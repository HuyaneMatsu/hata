...
class SubterraneanHelpCommand(object):
    ...
    async def lookup_by_name(self, client, message, name):
        ...
        By cases calls the following methods:
        - ``.list_category`` : If `name` matches a catgeory's name.
        - ``.show_string_description`` : If `name` matches a command's name and the command's `description.` is `str`
            instance.
        - ``Command.description`` : If `name` matches a command's name and the command's `description.` callable.
        - ``.show_autogenerated_description`` : If `name` matches a command's name, but the command has neither `str`,
        ...
        try:
            ...
            await self.list_category(client, message, category)
            return
